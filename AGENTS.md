...
- **v9 Module Path**: This project uses `github.com/ai8future/chassis-go/v9` as its Go module path. All internal imports use the `/v9` suffix. When adding new packages or files, always use `github.com/ai8future/chassis-go/v9/...` for import paths. When referencing this module in documentation, use the `/v9` path. The `chassis.RequireMajor(9)` call is required in all test files and service entrypoints.
